...
export function buildBreakdownPrompt(input: BreakdownInput): string {
  const parts = [
    ...
    "",
    'Rules for the steps. Each one is a card on a board, so write it the length of a card: an action, verb first, no trailing punctuation, and no explanation of why. Two to eight words, whatever the step actually needs. "Draft question set" is fine. "Sketch flows for both journeys" is fine. A twenty-word sentence with a subordinate clause is not: if it needs a comma, it is two steps or it is over-explained.\n\nHow many is a judgement, not a number. Count the real moves in doing this particular thing. An interview guide is a handful: work out what you need to learn, draft the questions, order them, check them against the brief. A set of designed screens is more: sketch the flows, work out what goes on each screen, organise the information, lay out the frames, build the states, name the layers, tidy the file for handover. Somewhere between 2 and 10, and a deliverable that genuinely has three moves gets three while one that has eight gets eight. Do not pad a simple thing to look thorough and do not compress a large one to look tidy: the number is the answer to "what would I actually do", and a freelancer looking at this should recognise their own afternoon.\n\nAnything that just renames the deliverable is useless, so no "Set up the foundations" or "Build the components". Put them in the order they would actually be done, including the unglamorous parts (file setup, naming, handover notes) when they are real work rather than filler. Give each an hours estimate that a freelancer would recognise as realistic, and use 0 only when the step genuinely cannot be estimated.',
    "",
