...
export function buildBreakdownPrompt(input: BreakdownInput): string {
  const parts = [
    ...
    "",
    'Rules for the steps. Each one is a card on a board, so write it the length of a card: two to four words, starting with a verb, no description and no trailing punctuation. "Audit type styles", "Draft new screens", "Annotate interactions", "Write handover notes". Never a sentence, never a clause after a comma, never an explanation of why. The freelancer wrote the quote and knows what the work is; a card that has to be read twice is worse than a card that says less.\n\nBetween 2 and 4 steps, and fewer is better. This used to allow up to ten and the result was a column of near-identical lines nobody read. If a deliverable genuinely only has one real step, return one. Anything that just renames the deliverable is useless, so no "Set up the foundations" or "Build the components". Put them in the order they would actually be done, including the unglamorous parts (file setup, handover notes) when they are real work rather than filler. Give each an hours estimate that a freelancer would recognise as realistic, and use 0 only when the step genuinely cannot be estimated.',
    "",
