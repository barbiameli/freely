...
/**
 * The invoice as a react-pdf document, without rendering it.
 *
 * Split out from renderInvoicePdf so the editor can draw the same document in
 * the browser while it is being typed. The alternative was an HTML preview
 * built to look like the PDF, which is two templates that agree until the day
 * one of them is edited: the app has already been caught out twice by two
 * things describing one fact, and a preview that lies about the file the
 * client receives is the worst possible version of it.
 *
 * Pure: it builds an element and returns it. Nothing here touches the
 * filesystem or the database, so it is safe on either side of the wire.
 */
export function invoiceDocument(invoice: InvoicePdfData) {
