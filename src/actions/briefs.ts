  ...
  const project = await prisma.$transaction(async (tx) => {
    ...
    await tx.diaryEntry.create({
      data: {
        ...
        // The plan, and nothing in front of it. It used to open with "Kicking
        // off <the project title>, excited to get started. Here's what's ahead:"
        // which restates the heading, puts words in the freelancer's mouth, and
        // announces a list that is visibly a list. The timeline is the update.
        body: brief.timeline || "",
        autoGenerated: true,
      },
    });
